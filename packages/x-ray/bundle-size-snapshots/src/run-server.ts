...
export const runServer = ({ result, resultData }: TRunServer) => new Promise((resolve, reject) => {
          ...
          if (req.url === '/save') {
            ...
            await pAll(
              Object.keys(data).map((shortPath) => async () => {
                ...
                const snapshotDir = path.join(path.dirname(file), '__data__')
                const tarPath = path.join(snapshotDir, 'bundle-size-snapshots.tar.gz')
